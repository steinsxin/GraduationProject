...
def run_self_training(splits: PreparedSplits, device: torch.device, save_dir: Path, seed: int) -> Dict[str, object]:
    ...
    val_loader = make_loader(splits.val_X, splits.val_y.astype(np.float32), shuffle=False, augment=False, seed_offset=1)
    test_loader = make_loader(splits.test_X, splits.test_y.astype(np.float32), shuffle=False, augment=False, seed_offset=2)
    ...
    for round_idx in range(1, TOTAL_ROUNDS + 1):
        ...
        train_loader = make_loader(train_X, train_y, shuffle=True, augment=True, seed_offset=round_idx)
        model = LSTM_Model().to(device)
        optimizer = torch.optim.Adam(model.parameters(), lr=LR)

        model_path = save_dir / f"lstm_round_{round_idx}_seed_{seed}.pth"
        history = train_model(
            model=model,
    ...
    if best_history is not None:
        plot_history(best_history, save_dir / "best_round_training_curve_lstm.png")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="MIT-BIH LSTM single-model supervised training.")
    parser.add_argument(
        "--data-dir",
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.1,
        help="Fraction of train data used as initial true labels when building the split.",
    )
...
def main() -> None:
    ...
    print("=" * 64)
    print("Preparing MIT-BIH LSTM self-training dataset")
    ...
    result_path = save_dir / "mit_bih_lstm_results.json"
