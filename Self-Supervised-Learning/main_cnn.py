...
if __name__ == "__main__":
    ...
    weights_path = "weights/Bestmodel_CNN_ratio_1_99_fold_1.pth"
    ...
    print(f"预测概率统计: Min={all_probs.min():.4f}, Max={all_probs.max():.4f}, Mean={all_probs.mean():.4f}")

    # 筛选高置信度样本
    # 1 为房颤 (AFib), 0 为非房颤 (Normal)
    AFIB_TH = 0.55
    NORMAL_TH = 0.2
    ...
    results_data = {
        "accuracy": acc,
        "f1_score": f1,
        "pseudo_afib_count": len(X_afib),
        "pseudo_normal_count": len(X_normal),
        "afib_threshold": AFIB_TH,
        "normal_threshold": NORMAL_TH
    }

    json_path = os.path.join("results", "semi_supervised_results.json")
